...
class SessionBuilderTest extends TestCase{
    ...
    /**
     * @var SignalProtocolAddress $ALICE_ADDRESS
     */
    private static $ALICE_ADDRESS;
    private static function ALICE_ADDRESS(){
        if(!static::$ALICE_ADDRESS){
            static::$ALICE_ADDRESS = new SignalProtocolAddress('+14151111111',1);
        }
        return static::$ALICE_ADDRESS;
    }
    /**
     * @var SignalProtocolAddress $BOB_ADDRESS
     */
    private static $BOB_ADDRESS;
    private static function BOB_ADDRESS(){
        if(!static::$BOB_ADDRESS){
            static::$BOB_ADDRESS = new SignalProtocolAddress('+14152222222',1);
        }
    ...
    public function testBasicPreKeyV2(): void{
        ...
        $aliceSessionBuilder = new SessionBuilder($aliceStore,self::BOB_ADDRESS());

        $bobStore = new TestInMemorySignalProtocolStore();
        $bobPreKeyPair = Curve::generateKeyPair();
        $bobPreKey = new PreKeyBundle($bobStore->getLocalRegistrationId(),1,31337,$bobPreKeyPair->getPublicKey(),0,null,null,$bobStore->getIdentityKeyPair()->getPublicKey());
